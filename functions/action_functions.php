...
function addItem( $db, $name, $price, $discountPrice, $itemType ) {
    ...
    if( $numOfExistingItems > 0 ) {
        SetUserErrorMessage( "Item <b>$name</b> already exists" );
        return;
    } else {
        ...
        $priceDisplay = getPriceDisplayWithDollars( $price );
        $discountPriceDisplay = getPriceDisplayWithDollars( $discountPrice );

        sendSlackMessageToMatt("*Item Name:* $name\n*Price:* $priceDisplay\n*Discount Price:* $discountPriceDisplay\n*Type:* $itemType", ":heavy_plus_sign:", "NEW ITEM", "#b7ab1a");

        SetInfoMessage( "Item <b>$name</b> added" );
    }
}
...
function editItem( $db, $itemID, $name, $price, $discountPrice, $unitName, $unitNamePlural, $alias, $currentFlavor, $status, $expirationDate, $tag ) {
    ...
    SetInfoMessage( "Item <b>$name</b> edited" );
}
...
function editUser( $db, $userID, $slackID, $anonName, $inactive, $resetPassword, $isCoop, $isVendor ) {
    ...
    if( $resetPassword == true ) {
        ...
        SetInfoMessage( "Password for user was reset to \"$uniqueID\"." );
    }
    ...
    SetInfoMessage( "User edited successfully." );
}
...
function makeCommission( $db, $vendorID, $paymentMonth, $sodaAmount, $snackAmount, $note, $method, $sodaCommission, $snackCommission ) {
    ...
    if( $vendorID > 0 ) {
        ...
        SetInfoMessage( "Paycheck added successfully." );
    }
}
/**
 * @param $db SQLite3
...
function makePayment( $db, $userID, $paymentMonth, $sodaAmount, $snackAmount, $note, $method ) {
    ...
    if( $userID > 0 ) {
        ...
        if( $sodaAmount > $sodaBalance ) {
            ...
            SetUserErrorMessage( "This payment [" . getPriceDisplayWithDollars( $sodaAmount ) . "] is larger than the user\"s Soda Balance of [" . getPriceDisplayWithDollars( $sodaBalance ) . "]. Payment denied!" );
            return;
        }

        if( $snackAmount > $snackBalance) {
            error_log( "Bad Snack balance. Amount: [" . $snackAmount . "] Balance: [" . $snackBalance . "]" );
            SetUserErrorMessage( "This payment [" . getPriceDisplayWithDollars( $snackAmount ) . "] is larger than the user\"s Snack Balance of [" . getPriceDisplayWithDollars( $snackBalance ) . "]. Payment denied!" );
            return;
        }
        ...
        SetInfoMessage( "Payment added successfully." );
    }
}
/**
 * @param $db SQLite3
...
function restockItem( $db, $itemID, $quantity, $multiplier, $retailCost, $itemType, $expDate ) {
    ...
    $itemStatement = $db->prepare("SELECT Price, DiscountPrice, Name From Item where ID = :itemID");
    ...
    $retailCostDisplay = getPriceDisplayWithDollars( $originalRetailCost );
    sendSlackMessageToMatt("*Item:* $itemName\n*Quantity:* $originalQuantity\n*Cost:* $retailCostDisplay\n*Multiplier:* $multiplier", ":truck:", "RESTOCK ITEM", "#b7ab1a");

    SetInfoMessage( "Restocked <b>$itemName</b> with $quantity total units" );
}
...
function refillItem( $db, $isTest, $itemID_all, $addToShelf_all, $sendToSlack, $refiller ) {
    ...
    SetInfoMessage( "Refill was successful for " . count($itemID_all) . " items." );
}
...
function inventoryItem( $db, $itemID_all, $removeFromShelf_all, $auditAmount, $itemType ) {
    ...
    for ($i = 0; $i < count($itemID_all); $i++) {
        ...
        $statement = $db->prepare("SELECT " . getQuantityQuery() . ", Name FROM Item i WHERE i.ID = :itemID");
        $statement->bindValue( ":itemID", $itemID );
        $results = $statement->execute();
    ...
    foreach( $itemsInventoried as $itemName ) {
        $itemsForSlack .= "\n - $itemName";
    }

    sendSlackMessageToMatt(count( $itemsInventoried ) . " items" . $itemsForSlack, ":card_file_box:", "INVENTORY ITEM", "#b7ab1a");

    if( count( $itemsInventoried ) == 0 ) {
        SetUserErrorMessage( "No items were inventoried." );
    } else {
        SetInfoMessage( "Inventoried " . count($itemsInventoried) . " items $auditMessage" );
    }
}
...
function creditUser( $db, $isTest, $userID, $creditAmountInDecimal, $returnCredits ) {
    ...
    if( $returnCredits ) {
        if( $currentCredits - $creditAmountWholeCents < 0 ) {
            $validCredits = false;
            SetUserErrorMessage( "Failure! User only has $currentCredits cents -  cannot subtract $creditAmountWholeCents cents!" );
        }
    ...
    if( $validCredits ) {
        ...
        SetInfoMessage( "User credited successfully with " . getPriceDisplayWithDollars($creditAmountWholeCents) );
...
function purchaseItems( $db, $isTest, $userID, $itemsInCart, $cashOnly )
{
    if( count ( $itemsInCart ) == 0 ) {
        ...
        SetSystemErrorMessage( "No items were in the cart. Contact Matt Miles!" );
        return;
    }

    if( $userID == "" ) {
        error_log( "Something is wrong. User tried purchasing with a null user ID." );
        SetSystemErrorMessage( "User ID is null. Contact Matt Miles!" );
        return;
    }
    ...
    if( $inactiveUser == 1 ) {
        SetUserErrorMessage( "You cannot purchase items while you are inactive!" );
        return;
    }
    ...
    if( $itemQuantityValidationPassed ) {
        foreach ($itemsInCart as $itemID) {
            ...
            if ($shelfQuantity - 1 <= -1) {
                ...
                $purchaseMessage = $purchaseMessage . "- " . getHolidayItemName( $itemName ) . " (" . getPriceDisplayWithDollars($itemPrice) . ")\n";
            }
            $stopTimeItem = time();
    ...
    if( $errors == "" ) {
        ...
        if( !in_array( "U1FEGH4U9", $vendorsToNotify ) && $slackID != "U1FEGH4U9" ) {
        ...
        if( !$isTest ) {
            ...
            foreach( $vendorsToNotify as $vendorSlackID ) {
                // Send to vendors, but don't notify self
                if( $vendorSlackID != $slackID ) {
                    sendSlackMessageToUser($vendorSlackID, "*(" . strtoupper("$firstName $lastName") . ")*\n" . $purchaseMessage, ":shopping_trolley:", $itemType . "Stock - VENDOR RECEIPT", "#8623a6", $vendorSlackID, false);
                }
            }
        ...
        SetSystemErrorMessage( "Something went wrong - contact Matt Miles!! " . $errors );
    }
}
...
function defectItem($db, $itemID, $quantity ) {
    ...
    if( $quantity > $shelfQuantity ) {
        SetUserErrorMessage( "Error: Trying to defect out more than you have on the shelf of $shelfQuantity units." );
        return;
    } else {
 ...
 * @return string
 */
function registerUser( $db, $isTest, $username, $password, $passwordAgain, $firstName, $lastName, $phoneNumber, $captcha ) {
    if( strtolower( $captcha ) != "rsa" ) {
        SetUserErrorMessage( "You got the company question wrong. Nice try, robot." );
        return;
    } else if( $username == "" || $password == "" || $passwordAgain == "" || $firstName == "" || $lastName == "" ) {
        SetUserErrorMessage( "You must provide User Name, Password, First Name, and Last Name." );
        return;
    } else if( $password != $passwordAgain ) {
        SetUserErrorMessage( "Passwords did not match." );
        return;
    } else {
        ...
        if( $userExists ) {
            SetUserErrorMessage( "User <b>$username</b> already exists!" );
            return;
        } else {
            $statement = $db->prepare( "INSERT INTO User (UserName, Password, FirstName, LastName, PhoneNumber, DateCreated, SodaBalance, SnackBalance, SodaSavings, " .
                "SnackSavings, AnonName, IsCoop, Inactive, Credits, ShowDiscontinued, ShowCashOnly, ShowCredit, ShowItemStats, ShowShelf, SubscribeRestocks, ShowTrending, HideCompletedRequests) VALUES " .
                "( :username, :passwordSHA1, :firstName, :lastName, :phoneNumber, :date, 0.00, 0.00, 0.00, 0.00, 'Penguin', 1, 1, 0.00, 0, 0, 0, 1, 0, 0, 1, 0)" );
            $statement->bindValue(":username", $username );
            ...
            SetInfoMessage( "Registration complete! User <b>$username</b> has been created." );
        }
    }
}
