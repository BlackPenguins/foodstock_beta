<?php
    ...
    if($type == "CardArea" ) 
    {
        ...
        buildCardArea( $db, $itemType, $itemSearch );
    }
...
//        $db->exec( $infoSQL );
    ...
    else if( $type == "NotifyAllUsersOfPayment" ) {
        $statement = $db->prepare("SELECT UserID, SlackID, FirstName, LastName FROM User" );
        $results = $statement->execute();
        ...
        while ($row = $results->fetchArray()) {
            ...
            $openingLine = "Good morning, $name! It's a new month. Here is your FoodStock Balance:";
            $summaryForMatt .= notifyUserOfPayment( $db, $slackID, $userID, $name, $openingLine );
        }

        sendSlackMessageToMatt( "Notified these users about their balances:\n" . $summaryForMatt, ":credit:", "FoodStock Collection Agency", "#ff7a7a" );
    }
    else if( $type == "NotifySingleUserOfPayment" ) {
        $userID = trim($_POST["userID"]);

        $statement = $db->prepare("SELECT UserID, SlackID, FirstName, LastName FROM User where UserID = :userID" );
        $statement->bindValue( ":userID", $userID );
        $results = $statement->execute();
        log_debug( "Notifying User of Payment..." );

        while ($row = $results->fetchArray()) {
            $slackID = $row['SlackID'];
            $userID = $row['UserID'];
            $name = $row["FirstName"] . " " . $row['LastName'];
            $openingLine = "This a friendly reminder of your current outstanding Foodstock balance:";
            $summaryForMatt = notifyUserOfPayment( $db, $slackID, $userID, $name, $openingLine );

            sendSlackMessageToMatt( "Notified user about their balance:\n" . $summaryForMatt, ":credit:", "FoodStock Collection Agency", "#ff7a7a" );
    ...
    function notifyUserOfPayment( $db, $slackID, $userID, $name, $openingLine ) {
        $userProfile = new UserPaymentProfile();
        $userProfile->storePastYearPayments( $db, $userID, 12 );
        $purchaseMonths = $userProfile->getMonths();
        log_debug("Storing Payments[$name] - [" . count( $purchaseMonths ) . "]" );

        $totalOwedForAllMonths = 0;
        $totalSodaOwedForAllMonths = 0;
        $totalSnackOwedForAllMonths = 0;
        $monthTableRows = "";
        $monthsMissed = 0;

        foreach( $purchaseMonths as $purchaseMonth ) {

            /** @var $purchaseMonth PurchaseMonthObj */
            ...
            $totalBalance = $purchaseMonth->getTotalUnpaid();

            if( $totalBalance > 0 ) {
                $totalOwedForAllMonths += $totalBalance;
                $totalSodaOwedForAllMonths += $sodaTotalUnpaid;
                $totalSnackOwedForAllMonths += $snackTotalUnpaid;

                $monthTableRows .= "|  " . padCell( $monthLabel ) .
                                  "  |  " . padCell( getPriceDisplayWithDollars( $sodaTotalUnpaid ) ) .
                                  "  |  " . padCell( getPriceDisplayWithDollars( $snackTotalUnpaid ) ) .
                                  "  |  " . padCell( getPriceDisplayWithDollars( $totalBalance ) ) .
                                  "  |\n ----------------- ----------------- ----------------- -----------------\n";
                log_debug("Notifying User[$name] Month[$monthLabel] Soda Total[ $purchaseMonth->sodaTotal] Snack Total[$purchaseMonth->snackTotal] Soda Paid[$sodaTotalUnpaid] SnackPaid[$snackTotalUnpaid]");
                $monthsMissed++;
            }
        }

        if ( $totalOwedForAllMonths > 0 ) {
                $slackMessage = "$openingLine\n" .
                    "*Total Balance Owed:* " . getPriceDisplayWithDollars($totalOwedForAllMonths) . "\n" .
                    "*_Soda Balance:_* " . getPriceDisplayWithDollars($totalSodaOwedForAllMonths) . "\n" .
                    "*_Snack Balance:_* " . getPriceDisplayWithDollars($totalSnackOwedForAllMonths) . "\n\n MONTH BREAKDOWN:\n" .
                    "``` ----------------- ----------------- ----------------- ----------------- \n" .
                ...
                return "*$name:* " . getPriceDisplayWithDollars($totalOwedForAllMonths) . " - $monthsMissed unpaid months\n";
        }

        return "";
    }

    function padCell( $content ) {
        return str_pad( $content, 13, " " );
    }
?>
