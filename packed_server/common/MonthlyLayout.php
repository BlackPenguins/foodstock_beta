...
class MonthlyLayout {
    ...
    function __construct() {
        $this->userPaymentProfile = new UserPaymentProfile();
    }
    ...
    function draw( $db ) {
        ...
        benchmark_start( "BuildPurchase" );
        $this->buildPurchaseInformation( $db, $userID );
        benchmark_stop( "BuildPurchase" );
        ...
        benchmark_start( "PrintAllBillingRows" );
        foreach( $this->userPaymentProfile->getMonths() as $monthLink => $information ) {
            $this->printBillingMonthRow( $db, $month, $monthLink, $userID, $information, $name );
        }
        benchmark_stop( "PrintAllBillingRows" );
        ...
        benchmark_start( "PrintPurchaseHistory" );
        $this->printPurchaseHistory( $month, $monthHeader );
        benchmark_stop( "PrintPurchaseHistory" );
     ...
     * @param $userID
     * @param $information PurchaseMonthObj
     * @param $fullName
     */
    function printBillingMonthRow($db, $selectedMonthLink, $currentMonthLink, $userID, $information, $fullName ) {
        ...
        $currentMonthSodaTotal = $information->sodaTotal;
        $currentMonthSnackTotal = $information->snackTotal;
        $currentMonthSodaCashOnlyTotal = $information->sodaCashOnlyTotal;
        $currentMonthSnackCashOnlyTotal = $information->snackCashOnlyTotal;
        $currentMonthSodaCount = $information->sodaCount;
        $currentMonthSnackCount = $information->snackCount;
        $currentMonthSodaCashOnlyCount = $information->sodaCashOnlyCount;
        $currentMonthSnackCashOnlyCount = $information->snackCashOnlyCount;
...
SCRIPT;
    ...
    function storePurchaseHistory( $date, $itemDetailsID, $legacyDiscountPrice, $legacyPrice, $discountPrice, $price, $creditsUsed, $isCashOnly, $type, $itemName, $isCancelled, $retailCost, $itemID, $user ) {
        ...
        if( $isCancelled != 1 ) {
            ...
            $this->userPaymentProfile->storeItem( $itemName, $type, $balanceCost, $creditOrCashOnlyCost, $date );
        }

        $purchaseMonthLink = $this->userPaymentProfile->getMonthlyKeyByDate( $date );
