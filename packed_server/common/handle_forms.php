...
if( !$hasPermission ) {
    SetUserErrorMessage( "You do have permission to do this! Shame on you!" );
} else {
    ...
    purchaseItems( $db, false, $_SESSION['UserID'], $itemsInCart, $cashOnly );
        ...
        if( isset($_POST["Preferences_ShowTrending"]) ) {
         ...
         if( isset($_POST["Preferences_HideCompletedRequests"]) ) {
            $hideCompletedRequests = 1;
        }
        ...
        $anonNameUpdate = "";
        if( $total > 0 ) {
           SetUserErrorMessage( "The Anonymous Animal '$anonAnimal' is already being used by another user." );
        } else {
        ...
        $statement = $db->prepare( "UPDATE User SET ShowDiscontinued=:showDiscontinued, ShowCashOnly=:showCashOnly, ShowCredit=:showCredit, ShowItemStats=:showItemStats, " .
            "ShowShelf=:showShelf, SubscribeRestocks=:subscribeRestocks, ShowTrending=:showTrending, HideCompletedRequests=:hideCompletedRequests $anonNameUpdate where UserID = :userID" );
        ...
        $statement->bindValue( ":hideCompletedRequests", $hideCompletedRequests );
        $statement->bindValue( ":anonAnimal", $anonAnimal );
        $statement->bindValue( ":userID", $userID );
        $statement->execute();
        ...
        SetInfoMessage( "User Preferences saved." );
        ...
        if( $itemName == "" ) {
            SetUserErrorMessage( "Your request cannot be blank." );
        } else {
        ...
        $captcha = $db->escapeString($_POST["CAPTCHA"]);
        $phoneNumber = "";
        ...
        registerUser($db, false, $username, $password, $passwordAgain, $firstName, $lastName, $phoneNumber, $captcha);
        ...
        unlink( $filepath );
        SetInfoMessage( "Killed session at $filepath." );
    } else if (isset($_POST['AddItem'])) {
        addItem($db, $_POST["ItemName"], $_POST["CurrentPrice"], $_POST["CurrentDiscountPrice"], $_POST["ItemType"]);
    } else if (isset($_POST['EditItem'])) {
        ...
        editItem($db, $itemID, $name, $price, $discountPrice, $unitName, $unitNamePlural, $alias, $currentFlavor, $status, $expirationDate, $tag);
    } else if (isset($_POST['Restock'])) {
        ...
        restockItem($db, $itemID, $quantity, $multiplier, $retailCost, $itemType, $expDate);
    } else if (isset($_POST['Inventory'])) {
        ...
        inventoryItem($db, $itemID_all, $removeFromShelf_all, $auditAmount, $itemType);
    } else if (isset($_POST['Refill'])) {
        ...
        refillItem($db, false, $itemID_all, $addToShelf_all, $sendToSlack, $refiller);
    } else if (isset($_POST['SendBot'])) {
        ...
        sendSlackMessageToNerdHerdTest($botMessage, ":$emoji:", $botName);
    } else if (isset($_POST['EditUser'])) {
        ...
        editUser($db, $userID, $slackID, $anonName, $inactive, $resetPassword, $isCoop, $isVendor);
    } else if (isset($_POST['CreditUser'])) {
        $userID = trim($_POST["EditUserDropdown"]);
        $creditAmountInDecimal = trim($_POST["CreditAmount"]);
        $returnCredits = isset($_POST["ReturnCredits"]);

        creditUser($db, false, $userID, $creditAmountInDecimal, $returnCredits);
    } else if (isset($_POST['Defective'])) {
        $itemID = trim($_POST["DefectiveDropdown"]);
        $itemType = trim($_POST["ItemType"]);
        $quantity = trim($_POST["NumberOfUnits"]);
        defectItem($db, $itemID, $quantity);
    } else if (isset($_POST['Payment'])) {
        ...
        if( $sodaCommission > 0 || $snackCommission > 0 ) {
            makeCommission($db, $userID, $paymentMonth, $sodaAmount, $snackAmount, $note, $method, $sodaCommission, $snackCommission);
        } else {
            makePayment($db, $userID, $paymentMonth, $sodaAmount, $snackAmount, $note, $method );
        }
